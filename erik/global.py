...
class Actions:
    ...
    def focus_window_with(app_substring: str, title_regex: str):
        """Focuses the first window whose title contains the given substring."""
        for window in ui.windows():
            if app_substring in window.app.name and re.search(title_regex, window.title) is not None:
                try:
                    ...
                    return
        app.notify(f"No window found with app '{app_substring}' and title containing: '{title_regex}'")
